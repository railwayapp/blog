export const Background = () => (
  <div className="hidden md:block md:opacity-100 background absolute inset-0 pointer-events-none">
    <img
      src="/grid.svg"
      className="absolute top-0 left-0 transform scale-x-[-1] max-w-none"
      loading="lazy"
      decoding="async"
      aria-hidden="true"
    />
    <img src="/grid.svg" className="absolute top-[500px] right-0 max-w-none" loading="lazy" decoding="async" aria-hidden="true" />
    <img src="/blog.svg" className="absolute top-[0px] left-[-500px]" loading="lazy" decoding="async" aria-hidden="true" />
    <img
      src="/blog.svg"
      className="absolute top-[350px] right-[-450px] transform scale-x-[-1] rotate-[17deg]"
      loading="lazy"
      decoding="async"
      aria-hidden="true"
    />
  </div>
)
